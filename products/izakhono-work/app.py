...
async function showProject(name){S.project=name;S.file=null;$('projectName').value=name;setMode('build');try{const d=await api('/api/projects/'+encodeURIComponent(name)+'/tree');$('workspaceTitle').textContent=name;$('workspaceFiles').innerHTML=(d.items||[]).filter(x=>x.type==='file').map(x=>'<button class="workspace-file" data-path="'+esc(x.path)+'">'+esc(x.path)+'</button>').join('')||'<div class="small">No editable files yet.</div>';$('workspacePanel').classList.add('show');document.querySelectorAll('.workspace-file').forEach(el=>el.onclick=()=>openProjectFile(el.dataset.path));$('title').textContent='PROJECT · '+name;if((d.items||[]).some(x=>x.path==='index.html')){$('validationBox').textContent='Preview available. Validate or open preview from the build card.'}}catch(e){banner(e.message)}}
async function openProjectFile(path){if(!S.project)return;try{const d=await api('/api/projects/'+encodeURIComponent(S.project)+'/file?path='+encodeURIComponent(path));S.file=path;$('editorPath').textContent=path;$('editor').value=d.content||'';document.querySelectorAll('.workspace-file').forEach(el=>el.classList.toggle('active',el.dataset.path===path))}catch(e){banner(e.message)}}
async function saveProjectFile(){if(!S.project||!S.file){banner('Select a file first.');return}try{const d=await api('/api/projects/'+encodeURIComponent(S.project)+'/file',{method:'POST',body:JSON.stringify({path:S.file,content:$('editor').value})});$('validationBox').textContent='Saved '+S.file+'\nCheckpoint: '+d.checkpoint+'\nValidation: '+(d.validation.ok?'PASS':'NEEDS REPAIR');banner('');await refresh()}catch(e){banner(e.message)}}
async function validateProject(){if(!S.project)return;try{const d=await api('/api/projects/'+encodeURIComponent(S.project)+'/validate',{method:'POST',body:'{}'});$('validationBox').textContent='Validation: '+(d.ok?'PASS':'NEEDS REPAIR')+'\n'+(d.results||[]).map(x=>(x.ok?'PASS ':'FAIL ')+x.path+' · '+x.check+(x.error?' · '+x.error:'')).join('\n')}catch(e){banner(e.message)}}
async function checkpointProject(){if(!S.project)return;try{const d=await api('/api/projects/'+encodeURIComponent(S.project)+'/checkpoint',{method:'POST',body:JSON.stringify({label:'owner-manual'})});$('validationBox').textContent='Checkpoint created: '+d.checkpoint}catch(e){banner(e.message)}}
function closeWorkspace(){S.file=null;$('workspacePanel').classList.remove('show');$('editor').value='';$('editorPath').textContent='Select a file'}
async function refresh(){try{const s=await api('/api/status');const ready=s.backend==='online'&&s.model_installed!==false;$('runtime').innerHTML='<span class="version">v'+esc(s.version||'?')+'</span> · '+(ready?'Local model ready':(s.backend==='online'?'Model not installed':'Model backend offline'))+' · '+esc(s.model)+' · Builder '+(s.builder?'ready':'off');banner(ready?'':(s.backend==='online'?'Configured model '+s.model+' is not installed.':'Local model backend is offline.'))}catch(e){banner(e.message)}try{const d=await api('/api/conversations');$('threads').innerHTML=d.items.map(x=>'<div class="thread '+(x.id===S.id?'active':'')+'" data-id="'+x.id+'">'+esc(x.title||'Conversation')+'</div>').join('');document.querySelectorAll('.thread').forEach(el=>el.onclick=()=>load(el.dataset.id))}catch(e){}try{const d=await api('/api/projects');$('projects').innerHTML=d.items.map(x=>'<div class="project" data-name="'+esc(x.name)+'">'+esc(x.name)+' - '+x.files+' files</div>').join('');document.querySelectorAll('.project').forEach(el=>el.onclick=()=>showProject(el.dataset.name))}catch(e){}}
async function newChat(){closeWorkspace();const d=await api('/api/conversations',{method:'POST',body:JSON.stringify({})});S.id=d.id;setMode('chat');$('messages').innerHTML='<div class="empty" id="empty"><h1>Your work. <span>Your compute.</span></h1><p>Start a new owner-controlled conversation.</p></div>';$('title').textContent='New conversation';await refresh()}
...
$('send').onclick=send;$('newChat').onclick=()=>newChat().catch(e=>banner(e.message));$('chatMode').onclick=()=>{closeWorkspace();setMode('chat')};$('buildMode').onclick=()=>{closeWorkspace();setMode('build');$('input').focus()};$('closeWorkspace').onclick=closeWorkspace;$('saveFileBtn').onclick=saveProjectFile;$('validateBtn').onclick=validateProject;$('checkpointBtn').onclick=checkpointProject;$('input').addEventListener('keydown',e=>{if(e.key==='Enter'&&!e.shiftKey){e.preventDefault();send()}});$('token').value=localStorage.getItem('izakhonoWorkToken')||'';$('token').onchange=()=>{localStorage.setItem('izakhonoWorkToken',$('token').value);refresh()};$('file').onchange=async e=>{S.attachments=[];for(const f of [...e.target.files].slice(0,5)){let t=await f.text();S.attachments.push({name:f.name,text:t.slice(0,120000)})}$('files').innerHTML=S.attachments.map(f=>'<span class="file-chip">'+esc(f.name)+'</span>').join('')};refresh();
</script></body></html>'''
