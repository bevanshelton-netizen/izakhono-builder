...
if [[ -n "$PUBLIC_BUILD_ENV_FILE" ]]; then
  ...
  while IFS= read -r LINE || [[ -n "$LINE" ]]; do
    ...
    case "$KEY" in
      VITE_IZAKHONO_CORE_URL|VITE_IZAKHONO_PROJECT|VITE_IZAKHONO_PUBLIC_KEY|VITE_IZAKHONO_ANALYTICS_URL|VITE_BACKEND_MODE|VITE_IZAKHONO_POLICY_V2_CANARY|VITE_IZAKHONO_SCOPED_TABLES|VITE_IZAKHONO_TRUSTED_ACTIONS_CANARY|VITE_IZAKHONO_RELATIONAL_CANARY|VITE_IZAKHONO_READ_RPC_CANARY|VITE_APP_MODE|VITE_DEPLOYMENT_ENV|VITE_EXPECTED_SUPABASE_PROJECT|VITE_SUPABASE_URL|VITE_SUPABASE_PUBLISHABLE_KEY|VITE_KORA_URL|VITE_ALLEGRO_RADIO_STREAM_URL) ;;
